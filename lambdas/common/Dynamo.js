...
const Dynamo = {
    async query(params, TableName) {
        ...
        if (!data || !data.Items) {
            throw Error(`There was an error fetching the data for query ${JSON.stringify(params)} from ${TableName} ${JSON.stringify(data)}`);
        }
    ...
    async get(requestKey, TableName) {
        ...
        if (!data || !data.Item) {
            throw Error(`There was an error fetching the data with the params ${JSON.stringify(params)} from ${TableName}`);
        }
    ...
    async scan(params) {
        ...
        if (!data) {
            throw Error(`There was an error fetching all data from ${params.TableName}`);
        }
    ...
    async delete(requestKey, TableName) {
        ...
        if (!res) {
            throw Error(`There was an error deleteing item ${requestKey} in table ${TableName}`);
        }
