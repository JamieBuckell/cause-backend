...
const Notifications = {
    ...
    sendTransactionalEmail: async (data) => {
        if (data.fromAddress) {
            data.fromAddress = data.fromAddress+process.env.INTERNAL_ADDRESS_DOMAIN
        } else {
            data.fromAddress = process.env.FROM_ADDRESS
        }
        ...
        const emailParams = {
            Source: FromAddress,//process.env.FROM_ADDRESS, 
            ReplyToAddresses: [FromAddress],//[process.env.FROM_ADDRESS],
            Destination: {
                ToAddresses,
                BccAddresses,
            },
    ...
    sendInternalEmail: async (data) => {
        ...
        let FromAddress = data.fromAddress;
        let BccAddresses = ['email@jamiebuckell.co.uk'];
        if (!data.subject) {
            data.subject = 'CAUSE Foundation';
        }
        if (!data.pageTitle) {
    ...
    sendRawEmail: async (data) => {
        ...
        let bccAddress = 'email@jamiebuckell.co.uk, ' + process.env.INTERNAL_ADDRESS;// process.env.INTERNAL_ADDRESS;
