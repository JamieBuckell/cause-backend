...
const Functions = {
    hasPermission(event, permission) {
        if (event?.requestContext?.authorizer) {
            if (event.requestContext.authorizer?.claims['cognito:groups']) {
                const allGroups = event.requestContext.authorizer.claims['cognito:groups'].split(',');
    ...
    getUsersUniqueReference: (fullName) => {
        ...
        */
        let initials = fullName.replace("-", " ").match(/(\b\S)?/g).join("").toUpperCase()
        return initials;
    },
    buildFamilyRequestHTML(families) {
        ...
        if (families.length) {
            for (const [i, item] of families.entries()) {
                ...
                const preferences = JSON.parse(item.familyDetail);
                ...
                for (const p of preferences) {
                    ...
                    switch (p.toLowerCase()) {
                        case "single":
                            ...
                            break;
                        case "small":
                            rtnHTML += "Small Family - Maximum 3 family members";
                            break;
                        case "medium":
                            rtnHTML += "Medium Family - Maximum 5 family members";
                            break;
                        case "large":
                            rtnHTML += "Large Family - 6+ family members";
                            break;
                        default:
                ...
                rtnHTML += `
                ${item.additionalInfo ? '<br /><br /><strong>Additional Information:</strong><br />'+item.additionalInfo : ''}`;
            }
        }
        rtnHTML = `
    ...
    buildDonorAllocationHTML(families, familyMembers, hideHamperId) {
        let rtnHTML = ``;
        if (families.length) {
            if (families.length >= 5) {
                rtnHTML += 'Due to the number of families allocated, we have attached a CSV containing your family allocation details.<br /><br />';
            } else {
                for (const [i, f] of families.entries()) {
                    const familyDetail = f.familyDetail
                        .replace(/(<([^>]+)>)/gi, "")
                        .replace(/\n+/g, ',')
                        .replace(/\s\s+/g, '')
                        .replace(/,,+/g, ',')
                    ...
                    if (familyMembers.length) {
                        const currentFamilyMembers = familyMembers.filter(
                            (m) => m.familyId === f.requestId
                        );
                        ...
                        <tr>`;
                        rtnHTML += `
                            <td style="padding-right: 30px;">${fm.who}</td>
                            <td style="padding-right: 30px;">${fm.age} ${fm.age ? fm.ageType : ''}</td>
                            <td>${fm.additionalInfo ? fm.additionalInfo : ''}</td>`;
                        rtnHTML += `
                        </tr>`;
                        }
    ...
    async validateSubmission(submission, validations) {
        const standards = {
            'email': {
                pattern: new RegExp(/^([a-z\d\-\.\+]{1,50})@([a-z\d\-]{1,50})\.([a-z]{2,8})(\.[a-z]{2,8})?$/i),
        ...
        for (const v of validations) {
            const validationSubmission = await Functions.checkValue(v.key.split('.'), submission);
            ...
            if (
                ...
                if (
                    ...
                    if (!standards[v.type].pattern.test(validationSubmission.toString())) {
        ...
        switch (template) {
                ...
                rtnString += `
                    <div class="row">
                        <div class="col-12">
                            ...
                            ${m.additionalInfo ? '<br />Info: '+m.additionalInfo : ''}
                        </div>
                    </div>
                    `;
                }
            ...
            case 'workerDetail':
                ...
                if (params.telephoneNumber) {
                    rtnStr += ` - <a href="tel:${params.telephoneNumber}">${params.telephoneNumber}</a>`;
                }
                if (params.emailAddress) {
                    rtnStr += `<br /><a href="tel:${params.emailAddress}">${params.emailAddress}</a>`;
                }
                return rtnStr;
        }
        return "";
        ...
        switch (template) {
            case 'adminNewNominator':
                return {
                    ...
                    pageContent: `Hello,
                    <br /><br />
                    You've just had a new nominator register themselves to your team/organisation: ${params.companyName ?? ''}.
                    <br /><br />
                    Name: ${params.nominator.name ?? ''}<br />
                    Email: ${params.nominator.email ?? ''}
                    <br /><br />
                    Please log into the Web Portal (<a href="${params.appURL}">${params.appURL}</a>) and approve this nominator to ensure their nominations can be allocated.
                    <br /><br />
                    Kind Regards,<br />
            ...
            case 'donorRegister':
                return {
                    ...
                    <br /><br /><br />
                    <a href="${params.appURL}/subscription/verify/${encodeURIComponent(params.emailAddress)}?v=${params.donorHash}" style="padding:16px 47px;margin:0 auto;background:#009643;border-radius:100px;font-weight:600;line-height:20px;letter-spacing:0.2px;color:#ffffff;text-decoration:none;display: block;width: max-content;">Verify your email</a>`
                };
            case 'donorVerified':
                return {
                    subject: "CAUSE Foundation: Hamper request confirmation",
                    pageTitle: "Hamper request confirmation",
                    pageContent: `Thank you for registering your preferences and verifying your email, we have received your request and will email you as soon as we allocate your famil${params.familyCount > 1 ? 'ies' : 'y'}.
                    <br /><br />
                    Your preference request details are:
                    <br /><br />
                    <strong>Your Name:</strong> ${params.donorData.firstName} ${params.donorData.lastName}<br />
                    ${params.donorData.company ? '<strong>Company:</strong> '+params.donorData.company+'<br />' : ''}
                    <strong>Email Address:</strong> ${params.donorData.email}<br />
                    ${params.donorData.telephone ? '<strong>Contact Number:</strong> '+params.donorData.telephone+'<br />' : ''}
                    <br />
            ...
            case 'donorRegisterSubsequent':
                return {
                    subject: "CAUSE Foundation: Hamper request confirmation",
                    pageTitle: "Hamper request confirmation",
                    pageContent: `Thank you for registering for another hamper, we have received your request and will email you as soon as we allocate your famil${params.familyCount > 1 ? 'ies' : 'y'}.
                    <br /><br />
                    Your preference request details are:
                    <br /><br />
                    <strong>Your Name:</strong> ${params.donorData.firstName} ${params.donorData.lastName}<br />
            ...
            case 'organisationAdminAccount':
                return {
                    ...
                    <br /><br />
                    We are pleased to announce that nominations are now open for the 2022 CAUSE Christmas Hamper Campaign.
                    <br /><br />
                    In the past year, we have been working on a new bespoke nomination system. The nomination forms have been replaced by a facility to nominate directly online.
                    <br /><br />
                    ...
                    To log in to the portal you'll need to use the following details:<br />
                    Web Portal URL: <strong><a href="${params.appURL}">${params.appURL}</a></strong><br />
                    Username: <strong>${params.emailAddress}</strong><br />
                    Password: <strong>${params.TemporaryPassword}</strong>
                    <br /><br />
                    For security reasons, once you're logged in you will be asked to change your password.
                    <br /><br />
            ...
            case 'organisationAdminWelcome':
                return {
                    ...
                    pageContent: `<strong>Forward this email to anyone you wish to be a nominator for your team/organisation</strong><br /><br /> Hello,
                    <br /><br />
                    You have been identified as a contact for your organisation / team to provide family nominations to recieive a 2022 CAUSE Christmas Hamper.
                    <br /><br />
                    This year, we have a new bespoke nomination system in place. Using the link below you can now register your details and start nominating your families. The link is unique to your organisation/team, so please do not share it outside of your organisation/team.
                    <br /><br />
                    <a href="${params.nominatorRegisterLink}">${params.nominatorRegisterLink}</a>
                    <br /><br />
                    You don't need to add all the families at once - you are able to log back in and add other families later, however, we are on a very strict timetable this year so nominations will close on <strong>Friday 30 September 2022</strong> at <strong>6pm</strong>.
                    <br /><br />
                    This year, we are expecting another increase in nominations due to the current cost of living crisis and we are also fearful that our donor families may be struggling, leading to a reduction in our donations. With this in mind, please consider carefully if the family needs our help - our hampers are only intended for those in genuine financial need. 
                    <br /><br />
                    Finally, if the family has multi-agency involvement, please check with other agencies to ensure they are not nominated twice e.g by school and/or local authority teams.
                    <br /><br />
                    Hopefully, everything will go smoothly but please bear with us if you encounter any issues - everyone involved with CAUSE Foundation is a volunteer and has other commitments, but we will get back to you as soon as we can.
                    <br /><br />
                    Kind Regards,<br />
                    Christmas Hamper Campaign Team`
                };
            ...
            case 'userPasswordResetConfirm':
                return {
                    ...
                    <br /><br />
                    To log in you'll need to use the following password: <strong>${params.temporaryPassword}</strong>
                    <br /><br />
                    For security reasons, once you're logged in you will be asked to change your password.
                    <br /><br />
                    <a href="${params.appURL}">${params.appURL}</a>
            ...
            case 'donorFamilyAllocation':
                return {
                    subject: "CAUSE Foundation: Christmas Hamper Allocation",
                    pageTitle: "THANK YOU FOR SUPPORTING OUR CHRISTMAS HAMPER CAMPAIGN",
                    ...
                    ${this.buildDonorAllocationHTML(params.familyData, params.familyMemberData, true)}
                    <table width="100%" cellpadding="20px" cellspacing="0" style="width:100%;">
                    <tr><td style="padding:0 20px;">
                        <a href="${params.appURL}/pledge-confirmation?t=accept&e=${encodeURIComponent(params.emailAddress)}&v=${params.donorHash}${params?.campaignRequestId ? '&c='+params.campaignRequestId : ''}" style="display:block;padding:16px 47px;margin:0 0 0 auto;background:#009643;border-radius:100px;font-weight:600;line-height:20px;letter-spacing:0.2px;color:#ffffff;text-decoration:none;width: max-content;">Accept</a>
                    </td>
                    <td style="padding:0 20px;">
                        <a href="${params.appURL}/pledge-confirmation?t=change&e=${encodeURIComponent(params.emailAddress)}&v=${params.donorHash}${params?.campaignRequestId ? '&c='+params.campaignRequestId : ''}" style="display:block;padding:16px 47px;margin:0 auto 0 0;background:#FBA94A;border-radius:100px;font-weight:600;line-height:20px;letter-spacing:0.2px;color:#ffffff;text-decoration:none;width: max-content;">Change</a>
                    </td></tr>
                    </table>
                    <br /><br />
            ...
            case 'donorFamilyAllocationConfirmed':
                return {
                    subject: "CAUSE Foundation: Christmas Hamper Confirmation",
                    ...
                    Middlesbrough FC have kindly allowed us to once again use the <strong>SOUTH STAND</strong> for hamper drop offs on <strong>SATURDAY 26th NOVEMBER</strong>, <strong>SUNDAY 27th NOVEMBER</strong> between 10am and 5pm and <strong>MONDAY 28th NOVEMBER</strong> between 9am and 4pm
                    <br /><br />
                    ...
                    <br /><br />
                    <strong>Your Allocation</strong><br />
                    ${this.buildDonorAllocationHTML(params.familyData, params.familyMemberData, false)}
    ...
    getPDFTemplate (template, params) {
        let pdfContent = '';
        switch (template) {
            case 'hamperLabels':
                pdfContent = `
            ...
            case 'feedbackSlip': 
                pdfContent = `      
                    <div style="width:50%; float:left; padding:0 20px 30px; box-sizing: border-box; text-align:center;">
                        <p>
                            This hamper was provided by the kind and generous supporters of
                        </p>
                        <img src="https://portal.cause-foundation.org.uk/static/img/cause-foundation-logo.png" height="80px" style="margin:0 auto;" />
                        <p style="margin: 0.5em 0;text-align:left;">
                            <img src="###QR_CODE###" style=" max-width:200px; display:block; float:left; margin-right:20px;" />
                            If you would like to leave us anonymous feedback about your hamper, please scan the QR code with your phone</p>
                        <p>
    ...
    async generateAllocationEmails (donor, families, familiesMembers, standardTemplate, emailTemplate, includePDF, campaignRequestId) {
        ...
        const emailTemplateParams = await Functions.getEmailTemplate(emailTemplate, getEmailTemplateParams);
        ...
        for (const family of families) {
            ...
            const currentFamilyMembers = familiesMembers.filter(fm => fm.familyId == family.requestId)
    ...
    async generateConfirmedAllocationEmail (donor, families, familiesMembers, standardTemplate) {
        const emailTemplate = {
            familyData: families,
            familyMemberData: familiesMembers, 
        };
        const emailTemplateParams = await Functions.getEmailTemplate(emailTemplate, emailTemplate);
