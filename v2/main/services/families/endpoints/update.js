  ...
  try {
    ...
    if (parsed.nominations.length) {
      ...
      for (const [i, n] of parsed.nominations.entries()) {
        console.log("i, n", i, n);
        const familyId = n?.familyId ?? nanoid(12);
        const familyIndex = existingFamiliesData.findIndex(
          (f) => f.GSI2PK === familyId
        );
        const familyData = existingFamiliesData[familyIndex];
        ...
        familyData.members = familyMembersData;
        ...
        existingFamiliesData[familyIndex] = familyData;
      }
