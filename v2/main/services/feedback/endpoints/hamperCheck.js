...
exports.handler = async (event, context, cb) => {
    try {
        ...
        if (familyData && familyData[0]) {
            ...
            const hashCompare = Hashing.compare(hamperId, {salt: envSalt+familyId, hashedpassword: hamperHash})

            if (hashCompare) {
                return Responses._200({ hasFeedback: familyData?.feedback && familyData?.feedback !== '' });
            } else {
