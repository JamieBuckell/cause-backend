  ...
  try {
    ...
    if (!campaignId) {
      return Responses._400({
        messages: { error: "Campaign ID is required" },
      });
    }
    ...
    counts.hampersDirect = allFamilies.filter(
      (f) => f?.receiveStatus && f.receiveStatus.indexOf("direct-hamper") >= 0
    ).length;
    ...
    const sum = allFamilies.reduce((accumulator, object) => {
      ...
      return (
        accumulator +
        (object?.bagsReceived && object.bagsReceived != ""
          ? parseInt(object.bagsReceived)
          : 0)
      );
    }, 0);
