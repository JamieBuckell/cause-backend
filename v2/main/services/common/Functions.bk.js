...
const Functions = {
  ...
  async generateFamilyDynamics(members, csvContent = '') {
    let familyDynamics = [];
    for (const familyMember of members) {
      ...
      csvContent +=
        `"${family.reference}","${familyWho}","${familyMember.age}${
      ...
      if (includePDF) {
        pdfPages.push({
          ...
          replaceStrings: {
            ...
            "###FAMILY_DYNAMICS###": `${generateFamilyDynamics(currentFamilyMembers, csvContent)}`,
          },
        });
      }
