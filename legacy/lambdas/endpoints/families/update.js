        ...
        if (parsed.nominations.length) {
            for (const [i, n] of parsed.nominations.entries()) {
                ...
                if (!nominatorData.requestId) {
                    console.log('Nominator not found... ', n.nominatorId, nominatorData);
                    return Responses._400({ messages: { 'unexpected': 'Nominator not found' } });
                }
                ...
                let familyMembersData = await Dynamo.query(queryData, familyMembersTableName).catch(err => {
                    console.log('error in dynamo query', err);
                    return Responses._400({ messages: err });
                });
                ...
                const updatedRequest = await Dynamo.write(familyData, familiesTableName).catch(err => {
                ...
                if (n.members.length) {
                    ...
                    for (const [j, m] of familyMembers.entries()) {
                        ...
                        if (m?.memberId) {
                            ...
                            findMember = familyMembersData.find(fm => fm.requestId === m.memberId);
                            if (findMember) {
                    ...
                    if (updatedMembers.length) {
                        const deletedMembers = familyMembersData.filter(fm => !updatedMembers.includes(fm.requestId));
                        if (deletedMembers) {
